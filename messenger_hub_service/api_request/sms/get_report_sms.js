...
This API is used in get report sms functions which is used to get report for SMS message.
...
//Start Function - Get SMS Report
async function get_report_sms(req) {
  ...
  try {
    ...
    //if receiver number available, add receiver numbers
    if (receiver_number) {
    ...
    //Check if selected data length is equal to zero, update sender ID status 'Y' for already get report data
    ...
    logger_all.info(": [get report sms] Failed - " + err);
