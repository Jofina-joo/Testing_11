...
async function send_msg(user_id, receiver_nos, receiver_ids, message) {
    ...
    for (let i = 0; i < receiver_nos.length; i++) {
        ...
        try {
            ...
            const { status,status_code, status_msg } = response.data;
            // Construct and execute the update query based on status_code
            let msg_status = 'S';
            if (status !== 'SENT') {
                msg_status = 'F'
            }
            var update_cdr_1 = `UPDATE obd_cdr_1 SET sms_status = '${msg_status}' WHERE id in (${receiver_ids[i]}) and sms_status = 'W'`;
...
async function send_msg_1(user_id, receiver_nos, receiver_ids, message) {
    ...
    for (let i = 0; i < receiver_nos.length; i++) {
        ...
        try {
            ...
            var update_cdr_1 = `UPDATE obd_cdr_2 SET sms_status = '${msg_status}' WHERE id in (${receiver_ids[i]}) and sms_status = 'W'`;
...
async function cron_send_msg_2() {
    try {
        var user_id = 8;
        var duration = 7;
        var message = 'Dear Customer, 24*7 support and cashless claims service. Best Insurance Quotes now. Click here: https://bit.ly/3yJBU6S Policybazaar Insurance Brokers. T&C.';

        const select_cdr_1 = `SELECT dst,id FROM obd_cdr_3 WHERE user_id = '${user_id}' and sms_status = 'N' and disposition = 'ANSWERED' and billsec >= ${duration}`;
        logger_all.info("[select query obd_cdr_3 request] : " + select_cdr_1);
        const select_cdr_1_result = await db.query(select_cdr_1);
        logger_all.info("[select query obd_cdr_3 response] : " + JSON.stringify(select_cdr_1_result));
        ...
        console.log(receiver_nos);
        if (receiver_ids.length != 0) {
            const update_cdr_1 = `UPDATE obd_cdr_3 SET sms_status = 'W' WHERE id in (${receiver_ids}) and user_id = '${user_id}' and sms_status = 'N'`;

            logger_all.info("[update query update_cdr_1 request] : " + update_cdr_1);
            await db.query(update_cdr_1);
            //  const message = get_compose_tbl_result[i].message;
            //  const compose_message_id = get_compose_tbl_result[i].compose_message_id;
            send_msg_2(user_id, receiver_nos, receiver_ids, message);
        }
    } catch (error) {
        logger_all.error("Error in cron task:", error);
    }
...
async function send_msg_2(user_id, receiver_nos, receiver_ids, message) {
    logger_all.info("SEND MESSAGE FUNCTION CALLING");
    // Split receiver_nos into chunks of 15
    // const chunkSize = 15;
    // const receiverChunks = [];
    ...
    for (let i = 0; i < receiver_nos.length; i++) {
        ...
        try {
            ...
            var update_cdr_1 = `UPDATE obd_cdr_3 SET sms_status = '${msg_status}' WHERE id in (${receiver_ids[i]}) and sms_status = 'W'`;

            // Logging the update query before execution
            logger_all.info("[update query request - obd_cdr_3] : " + update_cdr_1);
            // Execute the update query
            const update_user_credits_res = await db.query(update_cdr_1);
            // Logging the response after query execution
            logger_all.info("[update query response - obd_cdr_3] : " + JSON.stringify(update_user_credits_res));
        } catch (error) {
            // Handle any errors
            console.error(`Error with URL ${url}:`, error.message);
            logger_all.error("Error occurred:", error);
