...
This API is used in update task rcs functions which is used to update sent report for RCS message.
...
//Start function to update task RCS
async function update_task_rcs(req) {
  //logger_all.info("hello TESTING ON UPDATE TASK RCS");
  var logger_all = main.logger_all
  var logger = main.logger
  logger_all.info(" [update task rcs] - " + req.body);
  ...
  try {
    ...
    var product_id = req.body.rcs_product_id;
    console.log(str)
...
//get user_master_id
 var get_usermaster_id = `SELECT user_master_id FROM user_management where user_id = '${user_id}'`
    ...
    var get_response = `SELECT * FROM ${DB_NAME}_${user_id}.compose_msg_status_${user_id} WHERE response_status = 'T' AND compose_message_id = '${compose_id}' AND sender_mobile_no = '${mobile_number}'`
    ...
    logger_all.info(JSON.stringify(data) + "RCS Calculation");

    txt_sms_count = data.messages;
    logger_all.info(txt_sms_count + " RCS count based");
    ...
    //Otherwise continue process
      ...
      for (var i = 0; i < list.length; i++) {
        try {
          ...
          if (array[1] == "") {
            ...
            //Update credits based on RCS Calculation
            ...
            var updatesenderlimits = `UPDATE sender_id_limits SET daily_used_credits = daily_used_credits + 1,total_used_credits = total_used_credits+1 WHERE user_rights_id = '${product_id}' and sender_id = '${active_sender_ID}'`;
          ...
          logger_all.info(": [update task RCS] Failed - " + err);
      ...
      var get_res_senderID = `SELECT sender_mobile_no from ${DB_NAME}_${user_id}.compose_msg_status_${user_id} WHERE compose_message_id = '${compose_id}'`
      ...
      var get_res_sendersts = `SELECT m1.sender_mobile_no, m1.compose_message_id FROM ${DB_NAME}_${user_id}.compose_msg_status_${user_id} m1 LEFT JOIN ${DB_NAME}.sender_id_master m2 ON m1.sender_mobile_no = m2.mobile_no AND m2.sender_id_status != 'P' WHERE m1.compose_message_id = '${compose_id}' AND m2.mobile_no IS NOT NULL`;
    ...
    logger_all.info(": [update task rcs] Failed - " + err);
    logger.info("[Failed response - Error ocurred] " + JSON.stringify({ response_code: 0, response_status: 201, response_msg: 'Error Occurred.' }))

    return { response_code: 0, response_status: 201, response_msg: 'Error Occurred.', request_id: req.body.request_id };
  }
}
//End function to update task RCS
