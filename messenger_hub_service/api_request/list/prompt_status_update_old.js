    ...
    try {
        ...
        if (prompt_status === 'R') {
            ...
            if (get_prompt_result.length > 0) {
                ...
                /*logger_all.info(`First File moving to server - scp ${env.Auth}@${env.Hostname}:${prompt_location} ${env.Prompt_Location}`);
                var { stdout, stderr } = await exec_wait(`scp ${env.Auth}@${env.Hostname}:${prompt_location} ${env.Prompt_Location}`);
                ...
                if (firsterrorLines.length > 0) {
                    ...
                    return { response_code: 1, response_status: 201, response_msg: 'scp failed' };
                }*/
            } else {
