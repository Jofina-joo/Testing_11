...
This API is used in update report functions which is used to update whatsapp report.
...
async function update_report(req) {
  ...
  try {
    ...
    function date_format(dateStr, original_date) {
      if (dateStr.toLowerCase().includes('yesterday')) {
        ...
        return moment(dateStr, 'DD MMMM, h:mm A').format('YYYY-MM-DD HH:mm:') + "00"
      }
    }
    ...
    //Otherwise continue process
      ...
      //Update Delivered and Read count
    ...
    logger_all.info(": [update report] Failed - " + err);
