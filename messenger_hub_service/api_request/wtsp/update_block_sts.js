...
async function update_block_sts(req) {
 ...
 logger_all.info("[Select query response] : " + JSON.stringify(get_usermaster_result))
    ...
    if (get_response_result.length != 0) {
      var update_data = `UPDATE ${DB_NAME}_${user_id}.compose_msg_status_${user_id} SET response_status = 'K', response_message = 'Failed because of blocked senderID',response_date = CURRENT_TIMESTAMP
    ...
    var update_campaign_sts = `UPDATE ${DB_NAME}_${user_id}.compose_message_${user_id} SET cm_status = 'B' WHERE compose_message_id = '${compose_id}'`;
    ...
    logger_all.info(": Update blocked Status Failed - " + err);
