      ...
      <div class="main-content">
        <section class="section">
          ...
          <div class="section-header">
            <h1>RCS Campaign List</h1>
            <div class="section-header-breadcrumb">
              <div class="breadcrumb-item active"><a href="dashboard">Dashboard</a></div>
              <div class="breadcrumb-item">RCS Campaign List</div>
            </div>
          </div>
          <!-- List Panel -->
  ...
  <div class="modal" tabindex="-1" role="dialog" id="approve-Modal">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form class="needs-validation" novalidate="" id="frm_sender_id" name="frm_sender_id" action="#" method="post"
            enctype="multipart/form-data">
            <div class="form-group mb-2 row">
              <label class="col-sm-5 col-form-label">RCS Sender ID <label style="color:#FF0000">*</label></label>
                  ...
                  if ($state1->response_status == 200) { ?>
                    ...
                    <? for ($indicator = 0; $indicator < count($state1->sender_id); $indicator++) {
                      if ($counter % 1 == 0) { ?>
                        ...
                        <? } ?>
                        <td>
                          <input type="checkbox" checked class="cls_checkbox1" id="txt_whatsapp_mobno_<?= $indicator ?>"
            ...
            <input type="hidden" class="form-control" name='product_name' id='product_name' value='RCS' />
          </form>
              <div class="form-group mb-2 row">
            <div class="col-sm-5 media_type" style="display:none"> <label class="col-form-label"><b>Media Type</b><label
                  style="color:#FF0000"></label></label>
            </div>
            <div class="col-sm-7 personalized_media " style="display:none"></div>
            <div class="col-sm-7" id="divVideo" style="display:none;">
            ...
            <div class="col-sm-5"> <label class="col-form-label"><b>Message</b><label
                  style="color:#FF0000"></label></label>
            </div>

            <div class="col-sm-7">
              <div id="msg_content"
                style="margin-top:10px;white-space: pre-line; word-wrap: break-word; word-break: break-word;"></div>
            </div>
    ...
    <script>
      ...
      function approve_campaign_list() {
        $.ajax({
          type: 'post',
          url: "ajax/display_functions.php?call_function=approve_campaign_list_rcs",
          dataType: 'html',
      ...
      function func_download_rc_no(encodedDatas, indicatori) {
        try {
          ...
          a.href = url;
          a.download = 'approve_campaign_rcs.csv';
      ...
      //popup function
      function func_save_phbabt_popup(indicatori, compose_message_id, campaign_name, user_id, message, msg_type, media_url, is_same_media) {
        ...
        $('.personalized_media').css('display', 'none');
        ...
        // Call remove_senderid function with the provided parameters
        $('#msg_content').html(message);
        if ((media_url && msg_type == 'VIDEO') || (msg_type == 'IMAGE' && media_url)) {
          if (msg_type == 'VIDEO' && is_same_media == 'true') {
            $('.media_type').css('display', 'block');
            $("#divVideo").css("display", "block");
            $('#divVideo video source').attr('src', media_url)
            $('#divVideo video')[0].load()
          } else if (msg_type == 'VIDEO' && is_same_media == 'false') {
            $('.media_type').css('display', 'block');
            $('.personalized_media').css('display', 'block');
            $('.personalized_media').html(' <div> Personalized Video<i class="fa fa-play-circle" style="font-size: 20px;margin-left:10px;"></i></div>');
          }
          if (msg_type == 'IMAGE' && is_same_media == 'true') {
            $('.media_type').css('display', 'block');
            $('#image_url').css('display', 'block');
            $('#image_url').css('width', '200px');
            $('#image_url').css('height', '200px');
            $('#image_url').attr('src', media_url);
          } else if (msg_type == 'IMAGE' && is_same_media == 'false') {
            $('.media_type').css('display', 'block');
            $('.personalized_media').css('display', 'block');
            $('.personalized_media').html(' <div> Personalized Image<i class="fa fa-image" style="font-size: 20px;margin-left:10px;"></i></div>');
          }*/
 var indicatoris, compose_message_ids, campaign_names, user_ids, messages, msg_types, media_urls;
      //popup function
      function func_save_phbabt_popup(indicatori, compose_message_id, campaign_name, user_id, message, msg_type, media_url, is_same_media) {
      ...
      $('#approve-Modal').find('.btn-success').on('click', function () {
        ...
        if (txt_whatsapp_mobno == "") {
          ...
          $.ajax({
            type: 'post',
            url: "ajax/message_call_functions.php?tmpl_call_function=send_approve_campaign_rcs" + send_code,
            contentType: false,
            processData: false,
            dataType: 'json',
            beforeSend: function () {
            ...
            success: function (response) { // Success
              if (response.status == '0' || response.status == 0) {
                ...
                setTimeout(function () {
                  window.location = 'approve_campaign_rcs';
      ...
      $('#reject-Modal').find('.btn-success').on('click', function () {
        ...
        $("#id_error_reject").html("Reason to reject must be between 4 and 50 characters.");
          ...
          $.ajax({
            type: 'post',
            url: "ajax/message_call_functions.php?tmpl_call_function=send_reject_campaign_rcs" + send_code,
            contentType: false,
            processData: false,
            dataType: 'json',
            beforeSend: function () {
