                ...
                <div class="card">
                  <form class="needs-validation" novalidate="" id="frm_contact_group" name="frm_contact_group"
                    ...
                    <div class="card-body">
                      ...
                      <!-- Media Type defined -->
                      <div class="form-group mb-2 row" id="id_personalised_video">
                      ...
                      <div class="form-group mb-2 row" id="id_upload_media" >
                        <label class="col-sm-3 col-form-label" style="float: left">Upload Media <label style="color:#FF0000">*</label>
                          <span data-toggle="tooltip"
                        ...
                        <div class="col-sm-4 file_image_header" style="float: left;">
                          <input type="file" accept="image/*, video/mp4" class="form-control"
                            ...
                            name="file_image_header" id="file_image_header" tabindex="5" required
 ...
 function chooseFile() {
  ...
  $("#current_text_value").text(trimmedText.length); // Update the length of trimmed text
    ...
    function preview_content() {
      ...
      $.ajax({
        type: 'post',
        url: "ajax/preview_call_functions.php?preview_functions=preview_compose_rcs",
        data: fd,
          ...
          success: function (response) { // Success
            if (response.status == 1) {
              ...
              if (response_msg_split[1] != '') {
                invalid_mobile_nos = "Invalid Mobile Nos : " + response_msg_split[1] + "This Mobile Numbers Are Invalid Mobile Numbers.Are You Sure The Compose RCS ?";
              }
...
console.log("Call submit")
      ...
      $("#id_error_display").html("");
      $('#textarea').css('border-color', '');
      $('#slt_group').css('border-color', '');
      $('#upload_contact').css('border-color', '');
      $('#file_image_header').css('border-color', '');
      $('#file_image_header_url').css('border-color', '');
...
$("#file_image_header").change(function () {
  ...
  if (fileSize > 3) { // Adjusted maximum size to 3 MB
    $("#id_error_display").html('Sorry, Upload Media file below 3 MB size');
    $("#file_image_header").val('');
    return false;
  }
});
...
$("#file_image_header_url").keyup(function(){
  var el_t = document.getElementById('file_image_header_url');
  var txt_header_name_length = $('.custom-width').text().length;
		if(txt_header_name_length < 0){
			$("#file_image_header").prop("required", true);
		}
		else{
			$("#file_image_header_url").prop("required", true);
			$("#file_image_header").prop("required", false);
		}
});
  </script>
</body>
