        ...
        <section class="section">
          ...
          <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
              ...
              <div class="breadcrumb-item">Detail Report</div>
            </div>
          </div>
...
function download_server_file(campaign_id){
 fetch('./php_script.php', {
  ...
  .then(data => {
    if (data.error) {
      console.error('File transfer failed:', data.details);
    } else {
  ...
  .catch(error => {
    ...
    function business_summary_report() {
      ...
      $.ajax({
        type: 'post',
        url: "ajax/display_functions.php?call_function=compose_obd_detailed&dates=" + dates + "",
        dataType: 'html',
        beforeSend: function () {
          $('.theme-loader').show();
        },
...
$(function () {
  ...
  $('input[name="dates"]').daterangepicker({
    ...
    startDate: oneWeekAgo,  // Set the default start date to one week ago
    endDate: today,         // Set the default end date to today
    maxDate: today,         // Set the maximum date to today
    minDate: moment().subtract(7, 'month'),  // Set the minimum date to seven months ago
    locale: {
      cancelLabel: 'Clear',
      format: 'YYYY/MM/DD'
    }
  ...
  $('input[name="dates"]').on('apply.daterangepicker', function (ev, picker) {
    ...
    var second_date = picker.endDate.format('YYYY/MM/DD');
    campaign_name();
    ev.preventDefault();
  });
